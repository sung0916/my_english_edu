...
export const useMysteryCardGame = () => {
    ...
    const nextQuestion = (finalScore?: number) => {
        // 인자로 받은 점수가 있으면 그걸 쓰고, 없으면 현재 State 사용
        ...
        if (currentIndex < questions.length - 1) {
            ...
            // 딜레이 후 입력 잠금 해제 (애니메이션 시간 확보)
            setTimeout(() => {
                setIsProcessing(false);
            }, 500);

        } else {
            // 마지막 문제라면 게임 오버 처리 (최신 점수 넘김)
    ...
    // 정답 체크 (View에서 호출, boolean 반환하여 효과음 재생 결정)
